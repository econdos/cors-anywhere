...
function proxyRequest(req, res, proxy) {
  ...
  try {
    if (proxyOptions.target.auth && req.headers.authorization && req.headers.authorization.toLowerCase().indexOf('digest') !== -1) {

      httpclient.request(proxyOptions.target, {digestAuth: proxyOptions.target.auth}, (err, data, other_res) => {
        if (err) {
          console.error(err)
          // use default basic auth
          proxy.web(req, res, proxyOptions);
        }
        Object.entries(other_res.headers).forEach(([key, value]) => {
          res.setHeader(key, value)
        })
        let origin = req.headers.origin || req.corsAnywhereRequestState.proxyBaseUrl
        if (origin) {
          res.setHeader('Access-Control-Allow-Origin', req.headers.origin || req.corsAnywhereRequestState.proxyBaseUrl)
          res.setHeader('Access-Control-Allow-Methods', 'OPTIONS, GET, POST, PUT, DELETE, PATH');
        }
        let reqHeaders = req.headers['Access-Control-Request-Headers'] || req.headers['Access-Control-Request-Headers'.toLowerCase()]
        if (reqHeaders) res.setHeader('Access-Control-Allow-Headers', req.headers['Access-Control-Request-Headers']);
        res.write(data)
        res.end()
      });
    } else {
